...
from STT.nlpProcessor import analyze_text, detect_intent, handle_intent
...
def stream_microPhone(stt_function, buffer_seconds=2, noise_profile_duration=3):
    ...
    with sd.InputStream(samplerate=fs, channels=1,
        ...
        try:
                                ...
                                try:
                                    ...
                                    if text and text.strip() and len(text.strip()) > 2:
                                        ...
                                        try:
                                            ...
                                            print(f"→ Verbs: {analysis.get('verbs', [])}")
                                            ...
                                            # ---------------- optional action ----------------
                                            try:
                                                handle_intent(intent, analysis)
                                            except Exception as ia_e:
                                                print("Error while handling intent:", ia_e)
                                        except Exception as nlp_e:
                                            print("NLP error:", nlp_e)
                                            traceback.print_exc()
                                    else:
                                        print("✗ No clear speech detected\n")
                                except Exception as proc_e:
                                    print("Error processing audio / STT:", proc_e)
                                    traceback.print_exc()
                            else:
            ...
            print("\n\nStopped listening by user (KeyboardInterrupt).")
        except Exception as e:
            print("\nError in stream_microPhone main loop:")
            print(e)
            traceback.print_exc()
